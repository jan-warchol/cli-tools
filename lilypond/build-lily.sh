...
# because that'd delete the repository and all work done with it.
cd $build
...
# Check whether $source is inside a repository (and discard actual
# error message produced by rev-parse by sending it to dev/null).
cd $source
git rev-parse 2> /dev/null
if [ $? != 0 ]; then
    # non-zero exit status of rev-parse means that this directory
    # (i.e. $source) isn't inside a (presumably lilypond) repository.
    # (This implies we're building outside $LILYPOND_GIT,
    # which means that $source=$build.)
    # 
    ...
    if [ $notArepo == 0 ]; then
        ...
        echo "But it already contains *subdirectories* with lily repositories."
    ...
    # we had sources, but they may need updating
    # (if they're a local clone of LILYPOND_GIT)
...
if [ "$3" != "" ]; then
    cd $source
    git checkout --quiet $3 || die;
else
    cd $LILYPOND_GIT
fi
commit=$(git rev-parse HEAD)
cd $source
git checkout --quiet $commit || die;
...
# give the user 5 seconds to check above information.
